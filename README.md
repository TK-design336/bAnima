...
[![bAnima サンプル動画](https://img.youtube.com/vi/aFtI2dQhVMQ/maxresdefault.jpg)](https://youtu.be/aFtI2dQhVMQ)
...
| **シーケンサー チャンネル** | VSE の SOUND ストリップが載っているチャンネルごとに、プロファイル・音素マッピング・感情マッピングを指定 |
| **プロファイル** | リップシンク解析用の MFCC プロファイル。同梱の男性/女性プロファイル、または Unity uLipSync からエクスポートした JSON を指定可能 |
| **検出 / Vol** | デバッグ表示。現在フレームで検出された音素と音量（log10 RMS） |
...
音声から推定した感情スコアに応じて、登録したシェイプキー / ポーズをブレンドします。`_High` スロットは感情強度が閾値を超えたときに追加で乗るバリアントです。
...
| パラメータ | 説明 |
|-----------|------|
| **範囲を指定** | オフ: シーンの `frame_start`〜`frame_end`。オン: 下の開始/終了フレームを使用 |
...
瞬きスケジュールを事前計算し、指定フレーム範囲にキーフレームを打ちます。リアルタイム再生と同じタイミングが再現されます。
...
胸・肩・肋骨など、呼吸に連動させたい部位を Morph / Pose で自由に登録します。Pose バインドでは **motion_amount**（回転 ±度 / 移動 ±m / スケール倍率）で変形量を指定します。
...
| **ランダムシード** | 揺れ・サッカードの再現性用 |

![呼吸・マイクロモーション](docs/images/breathing-micro-motion.png)
...
呼吸とマイクロモーションはそれぞれ独立してベイクできます。手続き的アニメーションをキーフレームに焼き付け、他のアニメーションと合成しやすくします。

![ベイク設定](docs/images/bake-settings.png)
...
出力: `banima-0.2.47.zip`（バージョンは `banima/__init__.py` の `bl_info["version"]` に同期）
